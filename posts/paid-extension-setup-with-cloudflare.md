...
I set up a quick system to accept payments and unlock the Chrome and Firefox extensions, _without_ running a server. That's important. Running servers is complicated and hard and distracts from actually shipping products.

Fortunately, with a little elbow grease and Cloudflare's generous free tier, we can take payments without a server. [^1]

[^1]: [Serverless](https://aws.amazon.com/serverless/) computing is such a godsend for us frontend devs. So many times, I've caught myself wishing to run _just a little_ backend code.
