...
1. Download a video with [yt-dlp](https://github.com/yt-dlp/yt-dlp/)
2. Convert it to be iOS-friendly with [ffmpeg](https://www.ffmpeg.org)
3. Upload it to Pocket Casts
...
I would also recommend signing up for [Pushover](https://pushover.net). It's a great service, especially here. You can start a video download, walk away, and get notified on your phone when it's done. If that's not appealing, feel free to fork the project and delete that code.

The first part is fairly straightforward only thanks to the tireless work of the `yt-dlp` maintainers. That program is fantastic and simplifies downloading YouTube videos _tremendously_.

The second part, converting the video with `ffmpeg`, is necessary to play the video on iOS. Pocket Casts will let you upload any `.mp4` within the size limit, but only videos encoded properly will actually play in app.

To upload the video to Pocket Casts, the Node script actually opens a browser window using [Puppeteer](https://pptr.dev). This library is normally used for end-to-end testing, but `download-video.js` uses it to click through Pocket Casts' public-facing website and upload the video file.
